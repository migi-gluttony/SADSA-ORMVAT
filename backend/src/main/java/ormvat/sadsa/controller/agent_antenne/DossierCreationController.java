...
public class DossierCreationController {
     ...
     */
    @PostMapping("/preview")
    public ResponseEntity<DossierSummaryDTO> previewDossier(@Valid @RequestBody CreateDossierRequest request) {
        try {
            log.info("Génération de l'aperçu du dossier SABA: {}", request.getDossier().getSaba());
            
            DossierSummaryDTO summary = dossierCreationService.generateDossierSummary(request);
