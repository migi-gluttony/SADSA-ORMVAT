<template>
  <div class="implementation-visits-container p-4">
    <!-- Header -->
    <div class="flex justify-content-between align-items-center mb-4">
      <div>
        <h1 class="text-2xl font-bold text-900">
          <i class="pi pi-cog mr-2"></i>
          Visites d'Implémentation - Service Technique
        </h1>
    ...
    <div class="grid mb-4" v-if="dashboardSummary">
      ...
      <div class="col-12 md:col-3">
        <Card>
          <template #content>
            <div class="text-center">
              <div class="text-3xl font-bold text-purple-500">{{ Math.round(dashboardSummary.avancementMoyenGlobal || 0) }}%</div>
              <div class="text-color-secondary">Avancement moyen</div>
            </div>
          </template>
        </Card>
      </div>
    ...
    <Card class="mb-4">
      <template #content>
        <div class="grid">
          ...
          <div class="col-12 md:col-2">
            <label class="block text-900 font-medium mb-2">Avancement</label>
            <div class="flex gap-2">
              <InputNumber 
                v-model="filters.pourcentageMin" 
                suffix="%" 
                :min="0" 
                :max="100"
                placeholder="Min"
              ...
              />
              <InputNumber 
                v-model="filters.pourcentageMax" 
                suffix="%" 
                :min="0" 
                :max="100"
                placeholder="Max"
    ...
    <div v-if="loading" class="text-center p-6">
      ...
      <div class="mt-3">Chargement des visites d'implémentation...</div>
    </div>
    ...
    <Card v-else-if="visits.length === 0" class="text-center">
      <template #content>
        ...
        <p class="text-600" v-else>
          Aucune visite d'implémentation n'est programmée.
        </p>
        <div class="mt-3">
          <Button 
    ...
    <Card v-else>
      <template #content>
        ...
        >
          ...
          <Column field="pourcentageAvancement" header="Avancement" :sortable="true">
            <template #body="slotProps">
              <div class="progress-cell">
                <div class="progress-info">
                  <span class="progress-percentage">{{ slotProps.data.pourcentageAvancement || 0 }}%</span>
                  <ProgressBar :value="slotProps.data.pourcentageAvancement || 0" class="mt-1" />
                </div>
              </div>
            </template>
          </Column>
    ...
    <Dialog 
      ...
      :maximizable="true"
    >
      <ImplementationVisitDetailComponent
    ...
    <CompleteImplementationVisitDialog 
...
// Components
import ImplementationVisitDetailComponent from '@/components/service_technique/ImplementationVisitDetailComponent.vue';
import CompleteImplementationVisitDialog from '@/components/service_technique/CompleteImplementationVisitDialog.vue';
...
const hasActiveFilters = computed(() => {
  return searchTerm.value?.trim() || 
         ...
         filters.dateDebut || 
         filters.dateFin ||
         filters.pourcentageMin !== null || 
         filters.pourcentageMax !== null;
});
...
async function loadDashboard() {
  try {
    const response = await ApiService.get('/service-technique/visits/dashboard');
...
async function loadVisits() {
  try {
    ...
    if (filters.pourcentageMin !== null) params.pourcentageMin = filters.pourcentageMin;
    if (filters.pourcentageMax !== null) params.pourcentageMax = filters.pourcentageMax;
    
    // Add sorting
    params.sortBy = sortField.value;
    params.sortDirection = sortOrder.value === 1 ? 'ASC' : 'DESC';

    const response = await ApiService.get('/service-technique/visits', params);
...
function navigateToDossiers() {
  router.push('/service_technique/dossiers');
}

async function viewVisitDetail(visit) {
  try {
    // Load detailed visit information
    const response = await ApiService.get(`/service-technique/visits/${visit.id}`);
...
function handleVisitCompleted() {
  ...
  toast.add({
    severity: 'success',
    summary: 'Succès',
    detail: 'Visite d\'implémentation finalisée avec succès',
    life: 4000
  });
}
...
async function exportToExcel() {
  try {
    const response = await fetch('/api/service-technique/visits/export', {
      method: 'POST',
      headers: {
    ...
    link.href = url;
    link.setAttribute('download', `visites_implementation_${new Date().toISOString().split('T')[0]}.xlsx`);
...
<style scoped>
.implementation-visits-container {
  min-height: 100vh;
